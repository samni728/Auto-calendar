...
管理员用Docker部署
  ...
  → 管理员首次登录，填写真实姓名、展示身份、酒店名称、时区和房间
  → 店长使用应用账号密码登录并连接外部日历或手动创建事件
  → 系统把事件映射到房间资源时间轴
  → 店长创建或调整预订/维修事件
  → 数据持久化并同步回来源系统
...
4. 管理员首次登录时必须完成资料与酒店初始化，系统不得预置虚构姓名、酒店、房间或订单；
5. PWA在手机、Pad和桌面断点下显示真实配置的房间，并支持日/周时间轴；
6. 新建一条维修事件后刷新页面仍然存在，拖动事件或两侧边缘后日期与时长正确保存；
7. 未授权邮箱、绕过Access的请求和未匹配域名都被拒绝。
...
* 时间轴采用Pointer Events统一鼠标、触控笔和触摸操作：拖动事件主体调整日期，拖动左右边缘调整时长；
* 时间轴至少提供“日”和“周”两种单位，拖动步长随当前单位变化，服务端仍负责日期合法性和房间冲突校验；
...
| 平台 | 视口/形态 | 必测流程 |
| --- | --- | --- |
| iPhone Safari/PWA | `390×844`附近、竖屏 | 登录、今日看板、触摸拖动/缩放事件、OAuth回跳 |
| Android Chrome/PWA | 常见手机视口 | 安装、登录、日/周切换、时间轴滑动与拖拽、表单和断网提示 |
| iPad Safari/PWA | `768×1024`、横竖屏 | 房间时间轴、触控调整日期和时长、切换方向后状态保留 |
...
* 第一个管理员首次登录必须完成引导，设置自己的姓名、展示身份、酒店名称、时区和初始房间；不得用产品示例数据冒充用户资料；
* `job_title`等展示身份与`workspace_admin`等系统权限角色分开保存，修改职位名称不得改变实际授权；
